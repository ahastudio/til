...
Devin이 생성한 PR에 리뷰 코멘트를 남기면
Devin이 해당 피드백을 읽고 코드를 수정한다.
GitHub PR 코멘트에서 `@devin`을 멘션하거나,
Devin 웹 UI에서 직접 지시를 추가할 수 있다.

### 한계

- 단순한 수정 요청(변수명 변경, 포맷 수정 등)은
  잘 처리하지만, 설계 수준의 피드백에는
  의도를 벗어난 수정을 할 수 있다.
- 리뷰 코멘트가 여러 파일에 걸쳐 있으면
  일부만 반영하거나 맥락을 놓치는 경우가 있다.
- 리뷰 반영 후 새 커밋을 추가하는 방식이라
  커밋 히스토리가 지저분해질 수 있다.

### CodeRabbit과 조합

[CodeRabbit](./coderabbit.md)은
PR이 생성되면 자동으로 코드 리뷰를 수행한다.
Devin이 PR을 만들고 CodeRabbit이 리뷰하는
파이프라인을 구성할 수 있다.
...
양쪽 모두 놓치는 문제가 생길 수 있다.
구조적 설계 오류, 비즈니스 로직 정합성,
보안 취약점 등은 사람이 반드시 확인해야 한다.

CodeRabbit의 리뷰 지시(Review Instructions)에
프로젝트 컨벤션과 중점 검토 항목을 설정해 두면
AI 생성 코드에 대한 리뷰 품질을 높일 수 있다.
