...
접근성 트리 전체를 보내면 15,000+ 토큰을 소비하지만,
Snapshot + Refs는 `@e1`, `@e2` 같은 결정적 참조로
요소를 특정하므로 토큰 효율이 극적으로 개선된다.
...
토큰을 기하급수적으로 소비한다.
agent-browser의 Snapshot + Refs는 이 문제를 구조적으로
해결한다.
93% 토큰 절감은 단순한 최적화가 아니라, 에이전트가 더
많은 단계를 하나의 컨텍스트 안에서 수행할 수 있게 만드는
질적 변화다.
이것은 "더 싸게" 가 아니라 "더 많이 할 수 있게"의 문제다.
