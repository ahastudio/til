...
### 스타 9만 8천 개가 품질 검증으로 제시된다

개요는 GitHub 스타 98.4k 이상으로 커뮤니티 검증을 받았다고 적는다.
이 문장이 이 저장소를 신뢰할 근거로 놓여 있다.

큐레이션 목록 저장소에서 스타는 다른 것을 뜻한다.
나중에 볼 만한 자료라는 표시이자 북마크이고, 목록의 폭과 주제의 인기에 따라 빠르게 쌓인다.
예제 코드가 실제로 동작하는지, 최신 SDK에서 깨지지 않았는지, 보안상 안전한지와는 거의 무관하다.

이 구분이 중요한 이유는 이 저장소의 용도가 읽는 것이 아니라 실행하는 것이기 때문이다.
100개가 넘는 예제가 각기 다른 프레임워크와 모델 SDK에 의존하는데,
그 의존성들은 몇 달 단위로 바뀐다.
몇 개가 현재 동작하는지, 자동 검증이 있는지가 실질적인 품질 지표인데 그 정보가 없다.

### 카테고리 구성을 진화의 증거로 읽는다

분석은 카테고리 구성이 LLM 앱 개발 패턴의 진화를 보여 준다고 적고,
단일 프롬프트에서 RAG로, RAG에서 에이전트로, 단일에서 멀티 에이전트로 이어지는 다섯 단계를 제시한다.
그리고 프로젝트 수를 그 근거로 든다.

그러나 이 저장소의 프로젝트 수는 생태계의 분포가 아니라 한 큐레이터의 관심사 분포다.
멀티 에이전트가 31개인 것은 멀티 에이전트가 실무에서 그만큼 쓰인다는 뜻이 아니라,
그 주제로 데모를 만들기 좋고 관심이 몰린다는 뜻일 수 있다.
데모 친화성과 실무 채택률은 이 분야에서 특히 크게 어긋나는 두 축이다.

진화라는 서술도 한 방향을 가정한다.
실무에서는 멀티 에이전트로 갔다가 조정 비용 때문에 단일 에이전트로 되돌아오는 경우가 흔하고,
RAG를 붙였다가 긴 컨텍스트로 대체하는 경우도 있다.
단계가 뒤로도 간다는 사실이 카테고리 수에는 나타나지 않는다.

### 예제가 데모와 제품 사이의 거리를 감춘다

VC 실사팀, 법률 자문팀, 채용팀, SEO 팀 같은 에이전트 팀 예제는
어떤 업무를 자동화할 수 있는가를 판단할 참고 틀을 준다고 평가된다.

여기에 빠진 것이 그 판단의 핵심이다.
이런 예제는 성공 경로 하나를 보여 주는 것이고, 실제 도입에서 시간을 잡아먹는 것은 나머지다.
입력이 예상과 다를 때의 처리, 에이전트가 근거 없이 단정했을 때의 검출,
결과가 틀렸을 때의 책임 소재, 반복 실행 시 비용과 지연, 감사 기록이 그렇다.
법률과 채용처럼 규제와 차별 이슈가 걸린 영역에서는 이 항목들이 기능보다 먼저 온다.

동작하는 데모가 있다는 사실이 자동화 가능성의 증거로 읽히면 판단이 어긋난다.
이 저장소가 잘 보여 주는 것은 무엇을 시도해 볼 수 있는지이지 무엇을 맡겨도 되는지가 아니며,
그 경계가 카테고리 이름에는 드러나지 않는다.

### 폭이 유지 비용으로 돌아온다

프레임워크 다양성이 학습 가치를 높인다는 평가는 맞는 면이 있다.
CrewAI, Phidata, OpenAI Agents SDK, Google ADK, AG2를 한자리에서 비교할 수 있는 자료는 드물다.

동시에 그 다양성이 이 저장소의 가장 큰 취약점이다.
프레임워크 다섯 개와 모델 제공자 예닐곱 개의 조합이 100개가 넘는 예제에 흩어져 있고,
각각이 독립적으로 낡는다.
어느 예제가 언제 검증되었는지 표시되지 않으면, 이용자는 실행해 보기 전에는 알 수 없다.

학습 자료로서 더 아쉬운 부분은 비교가 병치로만 제공된다는 점이다.
같은 문제를 다른 프레임워크로 풀어 볼 수 있다는 것과, 그 차이가 무엇을 의미하는지 정리되어 있다는 것은 다르다.
어느 프레임워크가 어떤 상황에서 유리한지에 대한 서술이 없으면,
독자는 다섯 벌의 코드를 각각 읽고 스스로 결론을 내야 하고 그것은 큐레이션이 덜어 주기로 한 일이다.
