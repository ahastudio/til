...
Show GN: [CodeBurn - AI 코딩 도구 토큰 사용량/비용 추적 TUI 대시보드 | GeekNews](https://news.hada.io/topic?id=29343)
...
GN에서도 이 지점이 가장 먼저 반응을 얻었다.
설정 파일의 비대함과 반복적인 파일 읽기를 잡아내는 것이 대규모 프로젝트에서 컨텍스트 관리에 지친
사람들에게 실질적인 도움이 되겠다는 것이다.[^gn-vtrapplepie]
비용 절감보다 컨텍스트 관리 쪽에 먼저 눈이 간다는 뜻인데, `optimize`가 겨누는 것이 지출이 아니라
설정 위생이라는 해석에 가깝다.

GN에서는 `openusage`라는 유사 도구도 언급됐다.[^gn-awbrg789]
보여주는 정보는 CodeBurn보다 적지만 토큰 사용량에 한해서는 UX가 더 낫다는 평이다.
이 대비가 이 범주 도구의 갈림길을 보여준다.
CodeBurn은 18개 도구를 한번에 지원하는 범위를 택했고, `openusage`는 하나를 잘 보여주는 쪽을 택했다.
사용자가 도구를 하나만 쓴다면 넓은 범위는 이점이 되지 않는다.
...
[^gn-vtrapplepie]: <https://news.hada.io/topic?id=29343#cid57164>
[^gn-awbrg789]: <https://news.hada.io/topic?id=29343#cid57157>
