...
### 카타에서 얻은 결론을 레거시 일반으로 확장한다

시연의 소재는 Gilded Rose Kata다.
연습용으로 설계된 문제이고, 그래서 세 기법이 특히 잘 듣는 조건을 모두 갖추고 있다.

그 조건을 세어 보면 분명해진다.
코드가 한 파일에 들어갈 만큼 짧고, 외부 의존성이 없고, 입력이 몇 개의 값으로 열거되며,
출력이 결정적이고, 실행이 즉시 끝나며, 동시성도 상태 공유도 없다.
승인 테스트가 전체 입력 공간을 사실상 덮을 수 있는 것은 이 조건들 때문이다.

실제 레거시 코드는 대개 반대편에 있다.
데이터베이스와 외부 API에 의존하고, 시간과 무작위성에 영향을 받고,
입력 공간이 열거 불가능하며, 한 번 돌리는 데 시간이 걸린다.
그런 코드에서 승인 테스트를 만들려면 의존성을 격리하는 작업이 먼저 필요한데,
그 작업이야말로 레거시 리팩터링에서 가장 어렵고 가장 위험한 부분이다.
카타는 그 부분을 이미 제거한 상태에서 시작한다.

### 현재 동작을 정답으로 고정하는 위험을 다루지 않는다

승인 테스트의 전제는 지금의 출력이 옳다는 것이다.
그래서 리팩터링 전후의 출력이 같으면 안전하다고 판단한다.

레거시 코드에는 아직 발견되지 않은 버그가 있다는 것이 그 코드가 레거시인 이유 중 하나다.
승인 테스트는 그 버그를 정답으로 승인하고, 이후의 모든 변경이 그 버그를 보존하도록 강제한다.
더 나쁜 경우는 나중에 누군가 그 버그를 고쳤을 때다.
승인 테스트가 실패하고, 실패의 의미가 회귀인지 개선인지는 테스트가 알려 주지 않는다.

이 문제를 다루는 방법이 없지는 않다.
승인 파일에 왜 이 출력이 맞는지 주석을 남기는 것, 의심스러운 케이스를 따로 표시해 두는 것,
리팩터링이 끝난 뒤 승인 테스트를 의도 기반 테스트로 교체하는 단계를 절차에 넣는 것이다.
시리즈가 승인 테스트를 발판이 아니라 안전망으로 제시하면서 이 출구 전략을 언급하지 않는 것은,
같은 기법을 몇 년째 유지하는 팀을 만들 여지를 남긴다.

### 성공한 경로만 남은 시연이 난이도를 감춘다

전달 형식이 스크린캐스트 3부작이다.
화면에서 각 단계가 이어지고, 도구가 제때 동작하고, 리팩터링이 의도대로 풀린다.

시연의 본질적 한계가 여기 있다.
막다른 길, 잘못 고른 추출 지점, 되돌린 변경, 이해가 어긋난 구간은 편집에서 사라진다.
남는 것은 정답 경로이고, 보는 사람은 그 경로를 재현하는 일이 얼마나 어려운지를 가늠할 수 없다.

이 문제가 특히 큰 이유는 이 시리즈가 가르치려는 것이 판단이기 때문이다.
어떤 조건문을 먼저 끌어올릴지, 어디까지 쪼갤지, 언제 멈출지는 기계적 규칙이 아니라 감각이고,
그 감각은 잘못 고른 경험에서 자란다.
성공한 경로만 보여 주는 형식은 기법의 절차는 잘 전달하지만 그 판단은 전달하지 못한다.

### 읽지 않고 리팩터링한다는 명제가 어디까지 성립하는지 밝히지 않는다

시리즈의 가장 눈에 띄는 주장은 코드를 이해하기 전에 리팩터링을 시작하라는 것이다.
구조를 정리하면 이해가 따라온다는 순서이고, 실제로 강력한 접근이다.

그러나 이 순서가 안전한 조건이 따로 있다.
변경이 동작을 바꾸지 않는다는 것을 테스트가 보증할 수 있어야 한다는 조건이다.
승인 테스트가 덮는 것은 관찰된 입력에 대한 출력뿐이므로,
성능 특성, 메모리 사용, 호출 순서, 잠금 획득 순서, 부수 효과의 타이밍은 보증 범위 밖이다.

이 항목들이 중요한 코드에서는 읽지 않고 시작하는 것이 위험해진다.
조건문 하나를 끌어올리는 변경이 평가 순서를 바꾸고, 그 순서에 의존하는 부수 효과가 있으면
출력은 같은데 시스템은 달라진다.
명제 자체를 부정할 필요는 없지만, 어떤 코드에서 이 순서를 택하면 안 되는지가 함께 제시되어야
독자가 자기 코드베이스에 적용할지 판단할 수 있다.
