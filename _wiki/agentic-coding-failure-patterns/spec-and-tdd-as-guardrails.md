...
관련: [결정론적 강제와 텍스트
지침의 경계](../harness-engineering/deterministic-vs-prompted.md)는 “테스트가
프롬프트보다 정확한 명세”라는 이 절의 관찰을 하네스 설계 전반의 원칙으로
일반화한다 — 판단을 텍스트 지침에 맡길지 검사 가능한 규칙으로 옮길지
가르는 기준이 바로 여기서 다룬 테스트-대-자연어 스펙의 구도와 같다.
