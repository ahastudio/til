...
저자의
[트윗](https://twitter.com/arpit_bhayani/status/2030573221955752234):

> 데이터베이스는 에이전트 공간을 위해 설계되지 않았다.
> 지원하는 데이터 타입이나 알고리즘의 문제가 아니라,
> 데이터베이스가 세워진 가정의 문제다. 모든 데이터베이스는
> 사람이 작성한 애플리케이션, 예측 가능한 쿼리, 의도된
> 쓰기, 짧은 커넥션, 루프 안에 앉아 있는 신중한 개발자를
> 전제로 설계되었다.

정겨울님의
[트윗](https://twitter.com/res_tin/status/2030996368899363282):

> agent를 db에 붙여 쓸 때 어떻게 써야 그나마 실수를
> 줄이고, 개선할 점을 찾고, 폭발 반경을 줄일 수 있는지
> 제시하는 글. 구조 자체를 가능하다면 soft delete,
> append only로 잡고 멱등적으로 쓰고 쿼리를 수행할 때
> 컨텍스트를 넣게하고 role에 따른 권한과 접근 범위를
> 세밀하게 선언하는 식.
...
### 안전성만 말하고 성능 비용은 말하지 않는다

방어 패턴에는 성능 비용이 따른다. 모든 쿼리에
