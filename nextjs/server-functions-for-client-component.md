...
API Route를 따로 만들지 않고 Server Functions만으로 처리하는 패턴입니다.
`'use server'` 지시어를 붙인 함수를 Client Component에서 직접 호출할 수 있어
타입 안전성을 유지하면서 보일러플레이트를 대폭 줄일 수 있습니다.
...
- **Server Function**: `'use server'`가 붙은 모든 비동기 함수 (상위 개념).
  React가 2024년 9월부터 공식화한 용어입니다.
- **Server Action**: Server Functions 중 `action` prop 또는
  `startTransition`으로 호출되는 서브셋입니다.

React가 먼저 이 둘을 구분했고, Next.js는 그 용어 체계를 따릅니다.
Next.js 문서는 아직 “Server Actions” 중심으로 서술되어 있지만,
React 공식 문서(`react.dev`)는 Server Functions를 상위 개념으로 정의합니다.

## 설정

Next.js 15 이상은 별도 설정 없이 사용 가능합니다.
...
변경(mutation)만 Server Functions에 맡기는 방식이 더 깔끔합니다.
...
- Server Functions 파일에는 `'use server'`를 파일 최상단에 선언합니다.
- 인자와 반환값은 직렬화 가능한 타입이어야 합니다 (함수, 클래스 인스턴스 불가).
- 민감한 로직이 서버에서만 실행되므로 클라이언트에 노출되지 않습니다.
- 내부적으로 HTTP POST 요청으로 변환되므로 네트워크 탭에서 확인할 수 있습니다.
...
API Route는 외부 클라이언트(모바일 앱, 서드파티)가 소비해야 할 때 유리합니다.

### 아키텍처 관점

모든 Client Component 통신을 Server Functions로 통일하면
“서버 로직이 어디에 있는가”가 명확해집니다.
...
API 스펙을 별도로 관리하지 않아도 타입스크립트 컴파일러가 일관성을 보장합니다.
소규모 팀이나 풀스택 Next.js 앱에서 개발 속도를 크게 높일 수 있습니다.
...
요청 횟수나 캐싱 전략에 주의하지 않으면 성능 문제가 생길 수 있습니다.
외부 파트너가 API를 소비해야 하는 경우에는 API Route와 병행해야 합니다.

## 인사이트

### “API Route 없는 풀스택”의 의미

Server Functions 패턴이 의미하는 바는 단순히 코드 절감이 아닙니다.
클라이언트-서버 경계를 “URL”이 아닌 “함수 호출”로 표현한다는 설계 철학의 변화입니다.
URL은 외부 계약(external contract)이고 함수는 내부 계약(internal contract)입니다.
외부에 공개할 필요 없는 통신에 URL을 붙이는 것은 과잉 설계였다는 성찰에서 나온 방향입니다.

### 경계의 점진적 이동

React Server Components가 “렌더링 경계”를 컴포넌트 단위로 옮겼다면,
Server Functions는 “통신 경계”를 함수 단위로 옮기고 있습니다.
두 흐름이 합쳐지면 Next.js 앱에서 개발자가 인식해야 할
클라이언트/서버 경계의 총량이 줄어들고,
대신 컴파일러와 프레임워크가 그 경계를 관리하게 됩니다.
