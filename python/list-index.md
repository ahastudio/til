...
주어진 값이 `list`의 어디에 위치하는지 알아냅니다.
찾을 수 없으면 `ValueError`가 발생하므로
먼저 `in`을 이용해서 `list` 안에 있는지 확인하면 안전합니다.
...
특정 범위 안에서 찾고 싶다면 `start`와 `end`를 쓸 수 있습니다.
늘 그렇듯이, `end`는 포함되지 않습니다.
...
`list` 외에도 `tuple`이나 `range` 같은 Sequence Type은
모두 `index`를 사용할 수 있습니다.
