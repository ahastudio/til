...
### 전부 한 사람의 진술이고 반대편이 없다

이 인터뷰가 다루는 사건은 개명을 둘러싼 분쟁, 크립토 커뮤니티의 조직적 괴롭힘,
그리고 Meta와 OpenAI의 인수 경쟁이다.
세 가지 모두 최소 두 당사자가 있는 일인데, 기록된 것은 한쪽의 말뿐이다.

인터뷰라는 형식의 한계이므로 그 자체가 결함은 아니다.
문제는 이 문서가 그 진술들을 사건의 경과로 정리한다는 점이다.
누가 무엇을 제안했고 어느 쪽이 먼저 압박했는지가 확인 없이 서술되면,
읽는 쪽에서는 증언과 사실을 구분할 표시가 없다.

특히 괴롭힘을 다루는 대목이 그렇다.
조직적이었다는 판단, 배후에 무엇이 있었다는 추정은 당사자가 겪은 일에 대한 해석이고
그 해석이 맞는지는 별도의 확인을 요구한다.
증언을 그대로 존중하면서도 확인되지 않았음을 표시하는 것은 양립 가능한 일이다.

### 비공개 협상 내용을 당사자가 공개한 자료다

Meta와 OpenAI의 인수 제안 내용과 그 차이가 이 인터뷰의 중심 소재 중 하나다.
인수 협상은 통상 비공개이며, 그 내용을 한쪽이 공개하는 데는 목적이 따른다.

가장 흔한 목적이 협상력이다.
두 회사가 경쟁 중이라는 사실이 알려지면 양쪽의 제안 조건이 올라가고,
아직 협상 중이라면 공개 자체가 협상의 일부가 된다.
결렬된 뒤의 공개라면 서사를 자기 쪽에서 먼저 정하는 효과가 있다.

그러므로 이 부분에서 읽어야 할 것은 두 회사의 전략 차이보다 공개의 시점과 의도다.
문서는 제안의 차이에서 각 회사의 전략적 성격을 도출하는데,
그 도출의 근거가 협상의 한 당사자가 선택적으로 공개한 내용이라는 사실이
분석 어디에도 단서로 붙지 않는다.

### 이해관계자의 예측이 중립적 전망처럼 인용된다

앱의 80%가 사라진다는 전망, 에이전트가 소프트웨어 산업을 재편한다는 진단,
스킬이 MCP보다 나은 인터페이스라는 주장이 이 인터뷰의 굵은 줄기다.

이 진술들의 화자는 에이전트 도구를 만들고 그 도구로 인수 제안을 받은 사람이다.
예측이 맞고 틀리고를 떠나, 예측이 실현될수록 화자의 프로젝트 가치가 커지는 구조다.
스킬과 MCP를 비교하는 대목은 더 직접적이다.
자기 프로젝트가 채택한 방식과 경쟁 표준을 비교하는 발언이기 때문이다.

문서의 분석은 이 주장들을 논리와 반론으로 다루면서도 화자의 위치를 계산에 넣지 않는다.
이해관계를 밝힌다고 주장이 틀려지지는 않지만,
같은 주장을 이해관계가 없는 사람이 했을 때와 같은 무게로 다룰 수는 없다.
특히 인용된 수치가 출처 없는 비율일 때는 그렇다.

### 두 번의 번역을 거친 인용이 그대로 논거가 된다

원자료는 중국어 매체의 기사이고, 화자는 영어로 말했을 가능성이 높다.
그렇다면 이 문서의 인용문은 영어에서 중국어로, 다시 한국어로 옮겨진 것이다.

기술 논의에서 이 경로는 위험하다.
스킬과 MCP의 차이처럼 용어의 뉘앙스가 논지를 결정하는 대목,
80%처럼 수치의 단위와 범위가 중요한 대목,
그리고 인수 협상처럼 표현의 강도가 사실관계를 좌우하는 대목이 모두 번역에 민감하다.

원문 영어 발언을 확인할 수 있는 다른 인터뷰나 본인의 공개 글이 있다면
핵심 인용 몇 개만이라도 대조해 두는 것이 이 문서의 신뢰도를 크게 높인다.
그 대조 없이 번역된 문장을 따옴표 안에 넣어 분석의 전제로 쓰면,
실제로는 번역자의 선택이었던 표현이 화자의 주장으로 굳는다.
