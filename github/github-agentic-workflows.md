...
마크다운에 자연어로 지시사항을 쓰면,
AI 에이전트가 GitHub Actions 위에서 실행한다.
YAML을 직접 작성할 필요 없이
"무엇을 해줘"라고 쓰면 된다.
...
위자드가 AI 엔진 선택(Copilot, Claude, Codex),
시크릿 설정, 워크플로우 배포까지 안내한다.
실행하면 저장소 활동을 분석한 리포트가
이슈로 생성된다.
...
`.github/workflows/`에 마크다운 파일을 만든다.
프론트매터에 설정, 본문에 자연어 지시사항을 쓴다.
...
에이전트는 기본적으로 읽기 전용이다.
쓰기 작업은 Safe Outputs라는 구조를 통해
별도 잡(job)에서 검증 후 실행된다.
에이전트가 직접 저장소를 수정할 수 없다.
