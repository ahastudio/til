...
**안전망**이자 **감시 체계**다.
[Verification Debt](verification-debt.md)는 이 안전망이 없을 때 무엇이 누적되는지 보여준다 — Beck의 TDD 규율은 사실상 검증 부채의 즉시 상환 정책이다.
호흡 없이 들숨만 반복하면 폐가 터지듯, 검증 없는 코드 생성은 부채를 폭발 임계점까지 쌓는다.
...
과제를 드러낸다.
[AI 시대 코딩 용어 비교](ai-coding-terms.md)는 이 비대칭을 한 줄로 압축한다 — “AI는 흡입(기능 추가)은 잘하고 호흡(리팩터링)은 못한다.”
이 비대칭이 복잡도를 쌓아 결국 AI 자신의 한계를 초과하게 만드는 구조는 Beck의 “복잡성 절벽”과 같은 현상을 다른 각도에서 본 것이다.
