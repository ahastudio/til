...
[프롬프트 엔지니어링](prompt-engineering.md)이 강조하는 “명시적 제약 조건 전달”과 같은 원리로, dry-run·비용 상한이 에이전트의 행동 공간을 안전하게 좁히는 장치 역할을 한다.
...
[mini-SWE-agent](mini-swe-agent.md)가 bash 하나에 모든 도구를 위임하는 극단과 비교하면, VibeFrame은 “bash 친화적이되 도메인 어휘를 추가한 CLI”라는 중간 지점에 서 있다.
