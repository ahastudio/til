...
효과적이다.
[RAG](rag.md)에서 정리한 “하이브리드 검색이 단일 임베딩 검색보다 안정적이다”라는 원칙을 QMD가 작은 SQLite 파일 안에서 실증한다.
...
캐싱으로 보완하는 패턴이다.
[macOS에서 로컬 코딩 에이전트 구축하기](local-coding-agent-macos.md)가 보여주는 “로컬 모델 + OpenAI 호환 엔드포인트” 패턴과 결합하면, 검색-리랭킹-에이전트 전체를 외부 API 없이 운영할 수 있다.
