...
[ACE: 에이전트가 경험에서 배우는 학습 루프](agentic-context-engine.md)가
Level 3을 파인튜닝 없이 Skillbook으로 구현한 사례다.
ACE의 Reflector와 SkillManager가 “규칙을 학습한다”는 Level 3의 정의에
정확히 대응한다.
...
5,000달러를 직접 태우며 입증한 결론과 정확히 일치한다.
[1인간 1에이전트 실험](one-human-one-agent.md)이 보여준 Cursor 스웜의
빌드 실패는 “데모에서 인상적이지만 프로덕션에서 부서지는” Level 4의
전형적 증상이다.
...
[Ralph Loop Setup](ralph-loop-setup.md)이 코드베이스 밖 컨텍스트를
`.agent/` 디렉터리에 패키징한 것이 Level 2를 가장 단순하게 구현한 예다.
PRD, STEERING.md, 작업 스펙이 모두 “왜 이 결정을 내렸는가”를 에이전트가
참조 가능한 형태로 외부화한 것이다.
