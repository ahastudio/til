...
원문: [AI Agent Workflow Orchestration Guidelines](https://gist.github.com/OmerFarukOruc/a02a5883e27b5b52ce740cadae0e4d60)
...
[검증 부채](verification-debt.md)가 경고하듯, AI 시대의 위험은 “동작하지 않는
코드”가 아니라 “테스트는 통과하지만 검증되지 않은 코드”다 — 증거 요구는
이 부채가 누적되지 않게 하는 게이트 역할을 한다.
...
확보한다.
그러나 [의도 부채](intent-debt.md)가 지적하듯, 파일에 기록할 수 있는 것은
“무엇을 했는가”와 “어떻게 했는가”에 가깝다 — “왜 그렇게 결정했는가”의
의도는 에이전트가 추론할 수 없으며, 추론한 근거는 종종 그럴듯한 날조다.
파일 외재화는 인지 부채를 줄이지만 의도 부채를 자동으로 해소하지는 않는다.
...
[골빈해커의 코딩 에이전트 프롬프팅 플로우](golbin-agent-prompting-flow.md)가
보여주는 17단계 검토 시퀀스는 이 원칙의 반대편 극단이다 — 신호를 줄이는 대신
검토의 밀도를 극단적으로 높이는 전략이며, 두 접근의 차이는 “에이전트를
얼마나 신뢰할 수 있는가”에 대한 베팅의 차이다.
