...
프롬프트 구조, 작업 분해, 검증 기준 —
이 메타 작업을 몇 주간 반복해야 비로소 돌아간다.
기술적으로 어려운 게 아니라 운영적으로 어렵다.
...
`Goal`/`Exit when`/`Step 1` 같은 다섯 칸 양식으로 비슷한 메타 작업을
카탈로그화한 것은 이 “운영적 어려움”을 표준 양식으로 굳히려는 시도다.
...
[장기 실행 에이전트](long-running-agents.md)가 “세션을 추가 전용 이벤트
로그로” 다루는 Anthropic의 패턴과 동일한 발상이다.
컨텍스트는 휘발성, 파일·이벤트 로그는 영속성으로 분리하는 것이
장시간 자율 실행의 공통 토대다.
...
[잠자는 동안 돌아가는 에이전트](agents-that-run-while-i-sleep.md)는
이 역할 전환을 더 밀어붙여 “인수 조건(Acceptance Criteria)을 먼저 쓰라”고
주장한다.
37시간 자율 실행의 신뢰성은 결국 통과 기준의 정밀도에서 나온다는 점에서
Ralph Loop의 “통과 기준 정의 능력”과 같은 결론에 도달한다.
