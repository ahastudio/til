...
[1인간 1에이전트 실험](one-human-one-agent.md)이 “수백 에이전트 스웜보다 1:1
협업이 낫다”고 결론 낸 것과 정확히 같은 결이다.
인간 오케스트레이터를 둔다는 선택은 단순한 자원 부족이 아니라 조정 비용이
지배적인 상황에서의 합리적 판단이다.
...
[멀티 에이전트 오케스트레이션의 세 가지 병목](multi-agent-orchestration-problems.md)
중 맥락 붕괴와 유령 위임을 인간이 직접 흡수하는 구조다.
자동 오케스트레이터가 풀지 못한 부분 실패 처리를 사람이 tmux 탭 색상으로
대신 감지한다.
