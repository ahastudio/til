...
Rails 8부터 기본 Active Job 백엔드로 포함된 데이터베이스 기반 Job Queue
시스템이다. Redis 없이 기존 데이터베이스만으로 백그라운드 작업을 처리한다.
...
- 트랜잭션 무결성 (Job과 앱 데이터가 같은 DB라 원자적 커밋 가능)

## Example: 주문 처리

Stripe 결제 후 확인 메일을 보내는 주문 처리 시스템. 같은 유저의 주문이 동시에
결제되지 않도록 동시성을 제어하고, 주문 생성과 Job 등록을 하나의 트랜잭션으로
묶어 데이터 정합성을 보장한다.
