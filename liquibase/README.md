...
데이터베이스 스키마를 수동으로 관리하면 여러 문제가 발생합니다.

- **환경 간 불일치**: 개발, 스테이징, 운영 환경의 스키마가 달라집니다.
  누가 어디에 어떤 ALTER TABLE을 실행했는지 추적할 수 없습니다.
- **배포 실수**: 수작업 SQL 실행은 누락이나 중복 실행을 유발합니다.
  금요일 저녁 배포에서 컬럼 하나를 빠뜨리면 주말이 사라집니다.
- **협업 충돌**: 여러 개발자가 동시에 스키마를 변경하면
  충돌을 감지할 방법이 없습니다.
- **롤백 불가**: 문제가 생겼을 때 이전 상태로 되돌리는 절차가
  없으면 장애가 길어집니다.

Liquibase는 이런 문제를 해결합니다. 스키마 변경을 코드처럼
버전 관리하고, 변경 이력을 추적하며, 자동으로 적용하고,
필요할 때 롤백합니다.

## Liquibase vs Flyway

Liquibase와 자주 비교되는 도구로 Flyway가 있습니다.

| 항목                 | Liquibase            | Flyway               |
|----------------------|----------------------|----------------------|
| 마이그레이션 형식    | SQL, XML, YAML, JSON | SQL, Java            |
| 롤백 지원            | 내장 (자동/수동)     | 유료 버전만 지원     |
| diff 기능            | 지원                 | 미지원               |
| 변경 추적 방식       | ChangeSet 단위       | 파일 버전 순서       |
| ChangeSet 조건부실행 | 지원 (precondition)  | 미지원               |
| Spring Boot 통합     | 지원                 | 지원                 |
| 라이선스             | Apache 2.0           | Apache 2.0 / 유료    |

두 도구 모두 SQL 기반 마이그레이션을 지원하므로 단순한
프로젝트에서는 차이가 적습니다. 롤백이 중요하거나
XML/YAML 기반 선언적 관리가 필요하다면 Liquibase가 적합하고,
SQL만으로 가볍게 쓰고 싶다면 Flyway도 좋은 선택입니다.
