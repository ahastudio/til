...
### 하나의 CLI 경험에서 일반 원칙을 뽑아낸다

일곱 가지 원칙은 모두 gws 하나의 설계 경험에서 도출되었다.
그리고 그 gws는 Google Workspace API를 감싸는 CLI다.

이 출처가 원칙의 모양을 상당 부분 정한다.
첫 번째 원칙인 원시 JSON 페이로드 우선은 감싸는 대상이 이미 JSON API일 때 자연스러운 선택이고,
두 번째인 런타임 스키마 인트로스펙션은 Discovery Document라는 기계 판독 규격이 이미 존재하기 때문에 가능하다.
여섯 번째인 다중 표면 지원 역시 그 단일 진실 공급원 위에 서 있다.

그러면 이 원칙들이 얼마나 넓게 적용되는지가 문제가 된다.
`git`, `ffmpeg`, `kubectl`, 빌드 도구처럼 감쌀 상위 규격이 없는 CLI에서는
스키마를 무엇으로부터 만들지가 첫 번째 난관이고, 그 난관이 이 글에는 등장하지 않는다.
제목이 당신의 CLI를 다시 쓰라는 명령형인 만큼, 어떤 CLI에는 해당하지 않는지가 함께 있어야 한다.

### 에이전트를 적대적 입력으로 규정하고 그 함의를 끝까지 밀지 않는다

입력 강화 절의 선언은 분명하다.
입력은 항상 적대적이라고 가정하라는 것이고, 에이전트는 신뢰받는 운영자가 아니라는 것이다.
경로 탐색, 제어 문자, 리소스 ID 오염, 이중 인코딩에 대한 구체적인 방어가 뒤따른다.

그런데 같은 글의 다섯 번째 원칙은 에이전트에게 스킬 파일로 지침을 주입하고 그것을 따르게 하는 설계다.
변경 작업에는 먼저 모의 실행을 하라, 쓰기 전에 사용자에게 확인하라, 목록 조회에는 필드 마스크를 붙이라는 지침이다.
이 지침들은 강제가 아니라 권고이며, 적대적이라고 가정한 상대에게 지키라고 부탁하는 규칙이다.

두 원칙이 같은 대상에 대해 서로 다른 신뢰 모델을 쓴다.
문자열 형식은 믿지 않으면서 행동 규약은 믿는 구조이고, 실제 사고는 후자에서 난다.
경로 탐색은 도구가 막을 수 있지만, 확인 없이 삭제를 실행하는 것은 스킬 파일이 막지 못한다.
확인이 필요한 명령을 도구 차원에서 기본 차단하고 명시적 플래그로만 열어 주는 설계가
같은 글의 논리를 일관되게 끝까지 민 형태인데, 글은 그 지점까지 가지 않는다.

### 사람 경험과 에이전트 경험이 직교한다는 주장에 비용이 빠져 있다

글은 두 경험이 반대가 아니라 직교한다고 정리한다.
발견 용이성과 예측 가능성은 서로 대립하지 않으므로 둘 다 추구할 수 있다는 것이다.

개념적으로는 맞지만 실행에서는 비용이 생긴다.
같은 기능을 두 가지 입력 형식으로 받고, 두 가지 출력 형식으로 내보내고,
사람용 도움말과 기계용 스키마를 함께 유지해야 한다.
100개가 넘는 스킬 파일은 그 자체가 유지보수 대상이며, API가 바뀌면 코드와 스키마와 스킬 파일이 함께 바뀌어야 한다.

이 비용을 감당할 수 있는 조건이 따로 있다.
스키마가 자동 생성되고, 스킬 파일이 그 스키마에서 파생되고, 불일치를 잡는 검사가 CI에 있는 경우다.
gws는 Discovery Document 덕분에 그 조건에 가깝지만, 대부분의 CLI는 그렇지 않다.
직교한다는 진술이 비용이 작다는 뜻으로 읽히면, 손으로 관리하는 두 벌의 문서를 만들고
그중 하나가 조용히 낡는 결과에 이르기 쉽다.

### 효과를 재는 수치가 하나도 없다

제안된 일곱 가지는 모두 그럴듯하지만, 어느 것도 효과가 측정되지 않았다.
스킬 파일 하나가 환각 한 번보다 싸다는 문장이 이 글에서 비용 효과에 가장 가까운 진술이다.

측정 가능한 항목이 없는 것도 아니다.
필드 마스크와 NDJSON 페이지네이션은 토큰 사용량으로 바로 잴 수 있고,
스키마 인트로스펙션은 에이전트가 올바른 호출을 만들기까지의 시도 횟수로 잴 수 있으며,
입력 강화는 차단된 요청의 종류와 빈도로 잴 수 있다.
모의 실행 기본화는 사고 건수로 잴 수 있다.

gws를 실제로 운영한 경험에서 나온 글이므로 이 수치들이 어딘가에 있었을 것이다.
그것이 제시되지 않으면 일곱 가지는 검증된 원칙이 아니라 잘 정리된 설계 의견으로 남고,
읽는 쪽에서는 어느 항목부터 손대야 효과가 큰지 판단할 근거를 얻지 못한다.
